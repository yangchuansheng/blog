...
- iterm
- iterm2
title: "iTerm2 配置-自定义配置和优化教程（上）"
subtitle: "关于 iTerm2 自身的配置和优化"
description: 本文主要介绍了 iTerm2 自身的配置和优化，打造一个完美的 iTerm2。
...
本教程总共分为三个部分：第一部分关于 iTerm2 自身的配置和优化；第二部分关于连接远程服务器的配置和优化；第三部分关于 zsh 的配置和优化。
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310133244.png)

### 背景透明与模糊设置

在 `Preperence → Profiles → Window → Window Appearance` 进行设置

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310133556.png)

### 窗口风格配置

在 `Preperence → Profiles → Window → Settings for New Windows` 进行设置

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310133836.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310134700.png)

然后点击 `Configure Hotkey Window`，设置快捷键。

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310135133.png)

为了只使用一个键，可以使用功能键作为快捷键，我选择的是 `F12`。

对于没有 `Touch Bar` 的 MacBook，我们可以这么设置，首先打开系统偏好设置，选择键盘设置。

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310140808.png)

选中 **将 F1、F2 等键用作标准功能键**。

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310141302.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/2020-03-10-%E6%88%AA%E5%B1%8F2020-03-10%E4%B8%8B%E5%8D%884.19.00.png)

"按下 Fn 键以" 选择 “显示F1、F2等键”：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/2020-03-10-%E6%88%AA%E5%B1%8F2020-03-10%E4%B8%8B%E5%8D%884.20.28.png)

这样就可以使用 `Fn+F12` 来快速打开终端。

同时在 `快捷键 → 功能键` 设置中添加 iTerm 应用，这样打开 iTerm2 窗口时就可以直接使用 `F12` 键来关闭窗口。

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/2020-03-10-%E6%88%AA%E5%B1%8F2020-03-10%E4%B8%8B%E5%8D%884.21.00.png)

到目前为止，悬浮终端的 `Profile` 就配置完成了，你可以按下你设置的 `HotKey` 来方便快速打开和隐藏命令行。如果你想新建标签页也用这个 Profile，可以将 `Hotkey Window` 设为默认的 Profile：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310151301.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310142837.png)

默认光标形状是酱紫的：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310143504.png)

更改设置之后光标形状就顺眼多了：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310144308.png)

## 3. 为自己代言

如果你要为自己代言，新建任何一个终端窗口都想打上自己的 `Logo`，可以在 `Preperence → Profiles → General` 进行设置。

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310164716.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310151509.png)

Title 选择 `Profile Name`，取消勾选 `Job Name`：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310151810.png)

这样每个标签页的标题都会显示为对应的 `Profile Name`：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310152117.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310154213.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310162954.png)

如果想统一配色，需要稍微调整一下配置，打开 `Appearence → General`，将 Theme 改为 `Minimal`：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310163214.png)

## 7. 关闭启动界面

如果你不想每次打开 iTerm2 都打开默认的窗口，也就是静默打开程序，可以在 `Appearence → General` 设置：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310154902.png)

勾选 `Exclude from Dock and ...`，更改设置之后下次你再重新打开 iTerm2 就会变成这个样子：

![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310155115.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310155736.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310155922.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310160409.png)
...
![](https://hugo-picture.oss-cn-beijing.aliyuncs.com/images/20200310160910.png)
